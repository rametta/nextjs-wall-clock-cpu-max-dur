...
Each route uses `maxDuration = 3` and either idles, burns CPU, returns immediately with long work in `after()` / `waitUntil`, uses a **floating promise**, or **`after()` with `Promise.all` plus a separate floating branch** (`/api/wall-clock-after-float-mix`). **`/`** links to all of them.
